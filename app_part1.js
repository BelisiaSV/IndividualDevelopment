// ============================================================
// BELISIA SV — SUPABASE CLIENT (geen npm nodig)
// ============================================================
...
const sb = {
  ...
  async signIn(email,pass) {
    ...
    if(d.access_token){this.token=d.access_token;this.userId=d.user.id;localStorage.setItem('sb_tok',d.access_token);localStorage.setItem('sb_usr',JSON.stringify(d.user));}
    return d;
  },
  async signOut() {
    await fetch(`${SUPABASE_URL}/auth/v1/logout`,{method:'POST',headers:this.h()});
    this.token=null;this.userId=null;localStorage.removeItem('sb_tok');localStorage.removeItem('sb_usr');
  },
  async me() { return (await fetch(`${SUPABASE_URL}/auth/v1/user`,{headers:this.h()})).json(); },
...
class SbQuery {
  ...
  _url(){let u=`${SUPABASE_URL}/rest/v1/${this.t}?select=${this._s}`;this._f.forEach(f=>u+='&'+f);if(this._o)u+=`&order=${this._o}`;if(this._l)u+=`&limit=${this._l}`;return u;}
  async get(){const r=await fetch(this._url(),{headers:sb.h({'Accept':'application/json'})});const d=await r.json();return{data:Array.isArray(d)?d:[],error:d.error||null};}
  async insert(data){const r=await fetch(`${SUPABASE_URL}/rest/v1/${this.t}`,{method:'POST',headers:sb.h({'Prefer':'return=representation'}),body:JSON.stringify(Array.isArray(data)?data:[data])});const d=await r.json();return{data:Array.isArray(d)?d[0]:d,error:d.error||null};}
  async update(data,id){const r=await fetch(`${SUPABASE_URL}/rest/v1/${this.t}?id=eq.${id}`,{method:'PATCH',headers:sb.h({'Prefer':'return=representation'}),body:JSON.stringify(data)});const d=await r.json();return{data:Array.isArray(d)?d[0]:d,error:d.error||null};}
  async del(id){const r=await fetch(`${SUPABASE_URL}/rest/v1/${this.t}?id=eq.${id}`,{method:'DELETE',headers:sb.h()});return{error:r.ok?null:'failed'};}
  async upsert(data){const r=await fetch(`${SUPABASE_URL}/rest/v1/${this.t}`,{method:'POST',headers:sb.h({'Prefer':'return=representation,resolution=merge-duplicates'}),body:JSON.stringify(Array.isArray(data)?data:[data])});const d=await r.json();return{data:d,error:d.error||null};}
}
...
const S = {
  ...
  players:[], kpis:[], principes:[], weekSessions:{},
  carriere:[], competenties:[], licenseTopics:[], reflecties:[],
  oefenvormen:[],
  activePlayer:null, activeWerkpuntId:null, activeLicenseTopic:null,
  weekOffset:0, charts:{}
};
...
function notify(msg='✓ Opgeslagen',col='#2EAA6A'){
  const n=document.getElementById('notif');n.textContent=msg;n.style.borderLeftColor=col;n.style.display='flex';
...
async function doLogin(){
  ...
  if(!d.access_token){showErr(err,d.error?.message||'Login mislukt.');return;}
  await bootApp();
}
async function doRegister(){
  ...
  err.style.background='#D1FAE5';err.style.color='#065F46';err.textContent='✓ Bevestig je e-mail, log daarna in.';err.style.display='block';
}
...
async function doLogout(){await sb.signOut();location.reload();}
...
async function loadKpis(){
  ...
  const defs=[{id:'k1',name:'Passes geslaagd %',unit:'pct',default_val:70},{id:'k2',name:'Sprints p/90',unit:'num',default_val:60},{id:'k3',name:'Toplopen p/90',unit:'num',default_val:55},{id:'k4',name:'Afstand (km)',unit:'km',default_val:8}];
...
async function loadPrincipes(){
  ...
  const defs=[{label:'Passing in final third',color:'chip-blue'},{label:'Receiving under pressure',color:'chip-blue'},{label:'Finishing 1v1',color:'chip-green'},{label:'Schot op doel',color:'chip-green'},{label:'Pressing trigger',color:'chip-amber'},{label:'Compactheid verdediging',color:'chip-amber'},{label:'Balrecirculatie',color:'chip-blue'},{label:'Dieptepas timing',color:'chip-green'}];
...
async function loadCoachData(){
  ...
  S.carriere=carr?.length?carr:[{year:'2024–heden',title:'Performance Coach',club:'Belisia SV',description:'Seniorenvoetbal'},{year:'2020',title:'UEFA B Licentie',club:'KBVB Brussel',description:''}];
  S.competenties=(comp||[]).map(c=>c.name);
  for(const t of(topics||[])){const{data:vids}=await sb.q('license_videos').eq('topic_id',t.id).get();t.videos=vids||[];}
  S.licenseTopics=topics||[];
  S.reflecties=refs||[];
}
async function loadOefenvormen(){
  const{data}=await sb.q('oefenvormen').order('created_at').get();
  for(const o of(data||[])){const{data:h}=await sb.q('oef_load_history').eq('oefenvorm_id',o.id).order('session_date').get();o.loadHistory=h||[];o.principes=o.principes||[];o.load=o.load||{};}
  S.oefenvormen=data||[];
}
